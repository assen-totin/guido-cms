 ...
 */
function appLoadTemplate_admin_page() {
	appRun.logger.debug('Entering function appLoadTemplate_admin_page()...');

	// Form definition
	appRun.kvs.admin.page_form = {};
	appRun.kvs.admin.page_form.formId = 'F' + appUtilRandomUuid();
	appRun.kvs.admin.page_form.fieldLanguage = 'f' + appUtilRandomUuid();
	appRun.kvs.admin.page_form.fieldTitle = 'f' + appUtilRandomUuid();
	appRun.kvs.admin.page_form.fieldContent = 'f' + appUtilRandomUuid();
	var paramsForm = {
		id: appRun.kvs.admin.page_form.formId,
		div: 'admin_page_form',
		css: 'form-admin',
		exec: 'appAdminPageOnLoad',
		fields: {
			language: {
				type: 'SELECT',
				order: 10,
				...
				attributes: {name:'language', id: appRun.kvs.admin.page_form.fieldLanguage},
				extra: {
					options: [],
				},
				onChange: 'appAdminPageOnChangeLanguage',
			},
			title: {
				type: 'INPUT',
				order: 20,
				cssField: 'form-admin-div',
				cssInput: 'form-admin-input',
				attributes: {type: 'text', placeholder: _("Page title"), name:'title', id: appRun.kvs.admin.page_form.fieldTitle},
				validator: {
					enabled: true,
					validate: 'notempty',
			...
			content: {
				...
				cssField: 'form-admin-div',
				cssInput: 'form-admin-input',
				attributes: {name:'content', rows:4, cols:40, id: appRun.kvs.admin.page_form.fieldContent},
				extra: {
	...
	.done(function(data) {
		...
		var form = new guidoForm(paramsForm, function(error, result) {
			...
			// Add the page ID
			result.page = appRun.kvs.admin.page;

			var jqXHR = $.ajax({
				type: 'PUT',
				url: appConf.api.url + '/admin_page',
...
function appAdminPageDelete(id) {
...
function appAdminPageOnLoad() {
	if (appRun.kvs.admin.page_form.already_loaded)
		return;

	appRun.kvs.admin.page_form.already_loaded = true;
	appAdminPageOnChangeLanguage();
}

// On change of language drop down, change the value of the title and content
function appAdminPageOnChangeLanguage() {
	var elementLanguage = document.getElementById(appRun.kvs.admin.page_form.fieldLanguage);
	...
	.done(function(data) {
		...
		var elementTitle = document.getElementById(appRun.kvs.admin.page_form.fieldTitle);
		elementTitle.value = data[index].title;

		var elementContent = document.getElementById(appRun.kvs.admin.page_form.fieldTitle);
